...
public class TQuESTOperator extends DistanceOperator {
	...
	private static double m_maxthreshold = 0.0;
	
	private static double m_minthreshold = 0.0;
	
	private static double m_step = 0.0;
	...
	public void tuneOperator(Collection<Trajectory> trainset,
		...
		for (Iterator<Trajectory> it3 = trainset.iterator(); it3.hasNext(); ) {
		...
		for (double w = m_minthreshold; w <= m_maxthreshold; w+=m_step) {
			lg.fine("tuning with threshold:" + w);
			m_threshold = w;
		...
		lg.info("min:" + m_minthreshold + " max:" + m_maxthreshold);
